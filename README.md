...
- **Corrigido um bug real da planilha**: vários itens estavam cadastrados duas vezes, uma vez em português e outra em inglês (Aço/Steel, Ferro/Iron, Madeira/Wood, etc.), o que fazia a soma de "quanto falta" ficar errada. Agora esses pares são unificados automaticamente — a lista completa está na aba **Ajustes**.
...
- Alguns nomes de item vieram incompletos na planilha original (`??`, `Ball?`, `Roses ?`...) — ficaram como estavam, listados na aba Ajustes, pra você corrigir direto na tabela de Cidades quando souber o nome certo.
